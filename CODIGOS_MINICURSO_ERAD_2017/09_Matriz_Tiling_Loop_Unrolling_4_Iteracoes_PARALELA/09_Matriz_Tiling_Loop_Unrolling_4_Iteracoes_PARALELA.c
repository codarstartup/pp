...
void multiplica1(int m, int n, int **matA, int **matB, int **matC, int nucleos){
	int i, j, k, som;
	#pragma omp parallel for private(i, j, k) num_threads(nucleos)
	for (i=0;i<m;i++){
		for(j=0;j<n;j++){
			for(k=0; k<m; k+=4){
...
void multiplica3(int m, int n, int **matA, int **matB, int **matC, int nucleos){
	int i, j, k, som;
	#pragma omp parallel for private(i, j, k) num_threads(nucleos)
	for (j=0;j<m;j++){
		for(k=0;k<n;k+=4){
...
void multiplica4(int m, int n, int **matA, int **matB, int **matC, int nucleos){
	int i, j, k, som;
	#pragma omp parallel for private(i, j, k) num_threads(nucleos)
	for (k=0;k<m;k+=4){
		for(j=0;j<n;j++){
...
void multiplica5(int m, int n, int **matA, int **matB, int **matC, int nucleos){
	int i, j, k, som;
	#pragma omp parallel for private(i, j, k) num_threads(nucleos)
	for (i=0;i<m;i++){
		for(k=0;k<n;k+=4){
...
/*-------------------MATRIZES POR BLOCOS-------------------*/
//ORDEM I J K X Y Z
void multiplica7(int size, int n, int **A, int **B, int **C, int TILE, int nucleos){
	int i, j, k, x, y, z;
	#pragma omp parallel for private(i, j, k, x, y, z) num_threads(nucleos)
...
//ORDEM I J K X Z Y
void multiplica8(int size, int n, int **A, int **B, int **C, int TILE, int nucleos){
	int i, j, k, x, y, z;
	#pragma omp parallel for private(i, j, k, x, y, z) num_threads(nucleos)
	for(i = 0; i < size; i += TILE)
	    for(j = 0; j < size; j += TILE)
...
//ORDEM I K J X Y Z
void multiplica9(int size, int n, int **A, int **B, int **C, int TILE, int nucleos){
	int i, j, k, x, y, z;
	#pragma omp parallel for private(i, j, k, x, y, z) num_threads(nucleos)
	for(i = 0; i < size; i += TILE)
		for(k = 0; k < size; k += TILE)
...
//ORDEM I K J X Z Y
void multiplica10(int size, int n, int **A, int **B, int **C, int TILE, int nucleos){
	int i, j, k, x, y, z;
	#pragma omp parallel for private(i, j, k, x, y, z) num_threads(nucleos)
	for(i = 0; i < size; i += TILE)
		for(k = 0; k < size; k += TILE)	
...
int main (int argc, char *argv[]) {
	int **matA,**matB,**matC; 
	int m,n,i,j,k, algoritmo, tile, nucleos; // m(linhas) e n (colunas)
	int clockBegin;
	double timeElapsed;
	if(argc!=6){
		printf("Digite %s M_Linhas N_Colunas Interchange N_threads Tile\n\n", argv[0]);
		exit(0);
	}
	m=atoi(argv[1]);
	n=atoi(argv[2]);
	...
	switch (algoritmo){
		case 1: //ORDEM I J K O_QUARTO_MELHOR
			clockBegin = omp_get_wtime();
			multiplica1(m, n, matA, matB, matC, nucleos);
			timeElapsed = (omp_get_wtime() - clockBegin);	
			printf("%5lf\n", timeElapsed);
			break;
		case 2: //ORDEM J I K O_TERCEIRO_MELHOR
			clockBegin = omp_get_wtime();
			multiplica2(m, n, matA, matB, matC, nucleos);
			timeElapsed = (omp_get_wtime() - clockBegin);	
			printf("%5lf\n", timeElapsed);
			break;
		case 3: //ORDEM J K I O_SEXTO_MELHOR
			clockBegin = omp_get_wtime();
			multiplica3(m, n, matA, matB, matC, nucleos);
			timeElapsed = (omp_get_wtime() - clockBegin);	
			printf("%5lf\n", timeElapsed);
			break;
		case 4: //ORDEM K J I O_QUINTO_MELHOR
			clockBegin = omp_get_wtime();
			multiplica4(m, n, matA, matB, matC, nucleos);
			timeElapsed = (omp_get_wtime() - clockBegin);	
			printf("%5lf\n", timeElapsed);
			break;
		case 5: //ORDEM I K J O_PRIMEIRO_MELHOR
			clockBegin = omp_get_wtime();
			multiplica5(m, n, matA, matB, matC, nucleos);
			timeElapsed = (omp_get_wtime() - clockBegin);	
			printf("%5lf\n", timeElapsed);
			break;
		case 6: //ORDEM K I J O_SEGUNDO_MELHOR
			clockBegin = omp_get_wtime();
			multiplica6(m, n, matA, matB, matC, nucleos);
			timeElapsed = (omp_get_wtime() - clockBegin);	
			printf("%5lf\n", timeElapsed);
			break;
		case 7: //ORDEM I J K X Y Z - TILING
			clockBegin = omp_get_wtime();
			multiplica7(m, n, matA, matB, matC, tile, nucleos);
			timeElapsed = (omp_get_wtime() - clockBegin);	
			printf("%5lf\n", timeElapsed);
			break;
		case 8: //ORDEM I J K X Z Y - TILING
			clockBegin = omp_get_wtime();
			multiplica8(m, n, matA, matB, matC, tile, nucleos);
			timeElapsed = (omp_get_wtime() - clockBegin);	
			printf("%5lf\n", timeElapsed);
			break;
		case 9: //ORDEM I K J X Y Z - TILING
			clockBegin = omp_get_wtime();
			multiplica9(m, n, matA, matB, matC, tile, nucleos);
			timeElapsed = (omp_get_wtime() - clockBegin);	
			printf("%5lf\n", timeElapsed);
			break;
		case 10: //ORDEM I K J X Z Y - TILING
			clockBegin = omp_get_wtime();
			multiplica10(m, n, matA, matB, matC, tile, nucleos);
			timeElapsed = (omp_get_wtime() - clockBegin);	
			printf("%5lf\n", timeElapsed);
			break;
		default:
